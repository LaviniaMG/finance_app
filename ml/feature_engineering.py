...
def _add_account_type_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "statement_type" not in df.columns:
        raise ValueError("statement_type column is missing.")
...
def _add_series_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    grp = df.groupby(SERIES_KEYS, dropna=False)["target_amount"]
    ...
    df["rolling_mean_3"] = grp.transform(
        lambda s: s.shift(1).rolling(window=3, min_periods=1).mean()
    )
    df["rolling_mean_6"] = grp.transform(
    ...
    df["rolling_std_6"] = grp.transform(
        lambda s: s.shift(1).rolling(window=6, min_periods=2).std()
    )
    ...
    df["actual_vs_previous_year_abs"] = np.where(
        df["lag_12"].notna(), df["target_amount"] - df["lag_12"], np.nan
    )
...
def _add_ytd_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "ytd_rpt_amount" in df.columns:
        df["ytd_previous"] = df.groupby(SERIES_KEYS, dropna=False)["ytd_rpt_amount"].shift(1)
    else:
...
def _add_business_guardrail_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["history_count"] = df.groupby(SERIES_KEYS, dropna=False)["target_amount"].cumcount()
...
def _add_prev_rf_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds operational-model features based on the previous RF baseline.
    Expects columns prev_rf_mtd_rpt and prev_rf_ytd_rpt to already be present
    (populated by load_operational_baseline).
    """
    df = df.copy()

    has_prev_rf = "prev_rf_mtd_rpt" in df.columns

    if has_prev_rf:
        df["prev_rf_amount"] = np.where(
            df["statement_type"] == "BS",
            df["prev_rf_ytd_rpt"].fillna(0),
            df["prev_rf_mtd_rpt"].fillna(0),
        )
        # Delta: how much does the actual differ from what the prev RF predicted?
        df["actual_vs_prev_rf_delta"] = df["target_amount"] - df["prev_rf_amount"]
        df["actual_vs_prev_rf_pct"] = np.where(
            df["prev_rf_amount"].notna() & (df["prev_rf_amount"] != 0),
            df["actual_vs_prev_rf_delta"] / df["prev_rf_amount"].abs(),
            np.nan,
        )
        # Rolling mean of the RF-vs-actual deviation (how consistently over/under the RF is)
        df["rf_deviation_rolling_mean_3"] = (
            df.groupby(SERIES_KEYS, dropna=False)["actual_vs_prev_rf_pct"]
            .transform(lambda s: s.shift(1).rolling(window=3, min_periods=1).mean())
        )
    else:
        df["prev_rf_amount"] = np.nan
        df["actual_vs_prev_rf_delta"] = np.nan
        df["actual_vs_prev_rf_pct"] = np.nan
        df["rf_deviation_rolling_mean_3"] = np.nan
...
def build_feature_dataset(
    ...
    required_columns = [
        ...
        "fiscal_year", "period_id", "statement_type",
    ]
    ...
    if include_prev_rf or "prev_rf_mtd_rpt" in df.columns:
        df = _add_prev_rf_features(df)
...
def get_training_columns(include_prev_rf: bool = False) -> list:
    base = [
        "month_number", "year_number", "quarter",
        ...
        "run_rate_recent",
        "mom_change",
        "actual_vs_previous_year_abs", "actual_vs_previous_year_pct",
        "ytd_previous",
        "currency_effect_abs", "currency_effect_pct",
        "has_quarter_history", "has_half_year_history", "has_full_year_history",
        "volatility_ratio",
    ]
    if include_prev_rf:
        base += [
            "prev_rf_amount",
            "actual_vs_prev_rf_pct",
            "rf_deviation_rolling_mean_3",
        ]
    return base
